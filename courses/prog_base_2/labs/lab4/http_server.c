...
void parseHttpRequest(const char *request, http_method_t *http_method, char *url, char *message) {
    if (http_method) {
...
void http_server_start(http_server_t *http_server) {
    ...
    while (1) {
        ...
        if (!http_server_callback(http_server->middleware, http_method, url, request, response))
